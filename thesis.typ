...
Transformer的整体结构主要分为左右两端（如图2.5），左边为编码器（Encoder），右边为解码器（Decoder）。一层Transformer由一个编码器和解码器组成，而Transformer可以堆叠多层以提升模型的能力，而模型的训练难度也随之成倍增加。在《Attention is All You Need》原文中，作者权衡训练成本与效果最终使用了六层结构的Transformer。
