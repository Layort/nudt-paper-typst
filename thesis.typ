...
实验使用与@3.2 相同的实验环境训练改进的模型。为了验证跨帧Query查询机制和运动补偿机制的有效性，实验按顺序添加两个模块，称为"Ours-Query"(跨帧Query查询机制)和"Ours-Query+"(跨帧Query查询机制+运动补偿)。实验结果在@imporve_model_fig1 中。
...
从@imporve_model_fig1 中可以看到，跨帧Query查询机制模块显著增强了算法的鲁棒性。引入跨帧Query查询机制的改进算法相较于DETR3D全方面都有着明显的提升，尤其是mAP指标，代表算法的检测精度提升。这表明时空特征的融合有效增强了模型对遮挡目标的召回能力。随后引入的运动补偿机制相较于仅有跨帧Query查询机制的算法只有小幅度的提升，可能的原因在于数据集中无人机飞行速度较低且稳定，Transformer的解码器已经能够较好的学习并进行修正由于运动导致的坐标变换。所以运动补偿模块提升的效果并不明显。但总体来说两个模块都对算法有着积极的改进效果。
