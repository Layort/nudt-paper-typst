...
本章针对无人机视角中目标较小的问题，提出引入特征金字塔网络从而提升模型对于小目标的检测精度。该模型使用ResNet-101中的关键层级作为多尺度的特征图，并使用自上而下的特征融合方式。除此外，FPN还有横向连接机制，该机制是其实现语义传递的关键。模型融合目标的位置信息和语义特征，提升了小目标的检测精度。
...
在@fpn_result 中，基于FPN的改进模型在所有指标上取得了非常显著的进步，mAP提升了14.4%，NDS提升了10%，mATE提升了22%等，改进模型所有指标均为最优。由此可见，多尺度特征图对于3D目标检测效果的提升是全方位的，有效地提升了模型对于目标的感知能力。
