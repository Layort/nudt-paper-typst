...
计算机视觉是人工智能（Artificial Intelligence, AI）领域的重要研究方向和分支。在最近十年间，计算机视觉领域也得到了飞速发展。计算机视觉技术利用计算机来模拟生物的视觉感官系统，通过计算机的处理，传感器输入的信号转变成计算机可以理解的信息，为其他下游任务提供视觉基础，比如：目标检测、图像识别、人体分析等等。计算机视觉因此也应用于生活方方面面，如人脸识别、交通监控、光学字符识别（OCR）和工厂生产监控等。
...
尽管单阶段检测器在速度上占据优势，但其精度长期落后于两阶段方法。Lin等人于2017年通过类别不平衡理论分析（Class Imbalance Analysis）揭示根本原因：训练过程中大量简单负样本主导梯度更新，导致难样本学习不充分。为此RetinaNet引入了焦点损失函数（Focal Loss），通过动态调整难易样本的损失权重，使模型在COCO数据集上实现59.1% AP\@0.5，首次达到与两阶段方法相当的精度水平。

基于关键点检测的创新中，Law等人提出的CornerNet摒弃了传统锚框设计，转而通过角点热力图（Corner Heatmaps）与嵌入向量匹配（Embedding Matching）实现边界框生成，在COCO数据集上取得57.8%的AP\@0.5。该方法的无锚点（Anchor-free）特性显著降低了超参数调优难度，但角点分组依赖的后处理步骤仍存在计算冗余。Zhou等人进一步优化的CenterNet将目标建模为中心点热力图（Center Heatmaps），通过中心点直接预测物体的尺度与位置，在消除非极大值抑制（NMS）后处理的同时，将精度提升至61.1% AP\@0.5，展现了端到端检测框架的潜力。
...
自注意力机制与其他注意力机制不同，其在计算关注的部分时利用本身进行计算，得到的权重结果代表着序列元素之间的某种相关性，因此其对于输入序列的顺序并不敏感。自注意力的本质思想是一种查询和映射，Query通过Key来查询Value以获取其注意力权重，并将所有权重使用Softmax归一化得到权重百分比。
...
// 为了融合历史帧的时序信息，可以将上一帧Transformer最后输出的Query序列保留，依照模型给出的置信度分数从高到低截取若干个Query。如此便获取到目标在上一帧的位置，将其作为当前帧的Transformer的Query序列的一部分。而Tranformer的每一层解码器都会将Query经过交叉注意力机制进行修正，将Query的坐标逐渐从上一帧修正到当前帧的目标所在位置。 
...
多尺度特征融合算法能够利用卷积网络自下而上提取图像特征，随着图像分辨率的降低，特征图的语义信息逐渐增强，但随之位置信息也逐渐变弱。为了提升检测精度，多尺度特征融合算法将不同分别率下的图像特征进行不同方式的融合。优点在于结合了多尺度的图像信息，对于小目标，其在高分辨率特征图中更容易识别，提高了小目标的识别率和识别精度。使用多尺度特征融合的特征金字塔网络（Feature Pyramid Networkd，FPN）@fpn17  广泛应用于目标检测和计算机视觉算法。多尺度特征融合算法由于需要计算不同尺度下的目标的相关信息，计算量成倍提升。而FPN结合了多尺度特征融合和快速计算的优点，计算量增加较小且显著提升模型的检测精度。
...
在计算机视觉任务中，目标的尺度差异是影响任务性能的关键因素之一。早期的方法通过构建图像特征金字塔的方式在不同尺度下提取特征，实现了尺度不变性，但是缺点也很明显：首先是多尺度的图像特征提取导致计算量成几何倍数增长，大幅减慢了推理速度；其次是训练时的内存限制导致无法端到端优化图像特征金字塔，导致训练与测试阶段的特征不同；最后，深层网络限制了低分辨率下小目标特征的提取，导致小目标检测效果不佳。

为了解决这些问题，研究者提出了许多改进方案。W. Liu等人提出了SSD@ssd17，其通过卷积网络的不同层级直接构建特征金字塔，但仅对高层进行采样，丢失了底层的高分辨率信息。FPN在此基础上进行创新，将高层特征中的语义信息逐步向下传递，并且与横向的连接融合不同层级的特征信息，从而构建具有统一语义表示的金字塔结构。这种方法既能结合图像金字塔的多尺度信息，又避免了多尺度特征提取的计算冗余。
...
FPN的网络结构主要分为三个部分：自下而上的特征提取、自上而下的特征融合以及横向连接。

*自下而上的特征提取。*FPN没有单独设计网络逐层提取图像特征，而是借用其他的卷积神经网络，卷积网络的前项传播过程天然是金字塔型的特征提取过程。在前向过程中，特征图在经过某些层时大小并不会改变，在某些层则会改变。特征图大小不变的层统称为一个阶段，当特征图大小改变时进入下一个阶段。以ResNet为例，FPN选取的就是（C2,C3,C4,C4）四个卷积块作为四个阶段，其中C1到C2并没有改变特征的图的大小，因此没有选取。

*自上而下的特征融合和横向连接。*由于自上而下的特征融合和横向连接同时作用于特征图，此处合在一起介绍。通过自下而上的特征提取，FPN得到了4张不同尺度的特征图。在自上而下的特征融合中，FPN使用上采样（Upsampling）方法，是卷积的方向过程，也称作反卷积网络。通过上采样，FPN将高层的语义信息逐步融入下层，并且同时横向连接使用$1 times 1$的卷积将同一层的特征图进行处理后与上采样的结果相加，语义信息与高分辨率下的位置信息融合。横向连接使得浅层的细节信息与高层的语义信息有效结合在一起，形成了兼具高分辨率与强语义的特征图。
