...
无人机（Unmanned Aerial Vehicles）是指无人驾驶的飞行机器。近年来，由于其卓越的机动性，无人机被广泛部署在交通监控、精准农业、灾害管理和野生动物监控等领域。与传统监控摄像头相比，无人机提供更高的效率和适应性，对推进计算机视觉（Computer Vision）的众多应用至关重要。在这些应用中，鲁棒的对象检测和跟踪对于无人机的有效部署至关重要。然而，现有的无人机应用模型主要是针对传统的2D感知任务设计的，例如LAM-YOLO和Drone-TOOD，这限制了需要对环境进行3D理解的实际应用的发展。在基于视觉的机器人系统中，3D感知扮演着重要角色，使其能够处理2D感知无法胜任的复杂任务。虽然对于无人机来说，3D视觉仍然是相对较新的技术，但它提供了在3D环境中捕获对象的完整维度数据的能力。

随着自动驾驶技术的发展，研究人员开始关注3D视觉下多角度的目标检测技术，出现了DERE3D、PETR、PETRv2、BEVFusion等车载3D多视角目标检测模型。然而，目前尚未出现专注于无人机的3D多视角目标检测模型。同时，针对无人机3D多视角目标检测任务的数据集UAV3D日前发布，经过测试，先前的3D多视角目标检测模型在该数据集上表现均不理想，说明当前的研究结果并不适用于无人机领域。因此，有必要研究无人机的3D多视角目标检测模型，以提升无人机对于3D环境的理解能力。
...
无人机因价格便宜、使用方便、对人员安全以及操作人员培训简单而越来越受欢迎。这些优势，加上其的分辨率和强大的跟踪特性，促使它们在各种环境中的使用越来越多。无人机已被用于环境监测，包括空气污染、地表温度、洪水危险、森林火灾、道路表面损坏、地形监测、行人交通监测和灾害疏散。例如，许多人因为可以通过移动设备控制的先进产品而提高了生活水平。汽车技术通过提供关于交通的最新和精确信息来帮助驾驶员。无人机有多种规格、尺寸和配置。它们被归类为四大类别：固定翼、混合固定翼、单旋翼和多旋翼，同时考虑旋翼的数量。固定翼无人机适合于航空测量和绘图，因为它们稳定且续航时间长。混合固定翼无人机结合了自动化和手动滑翔，提供了可操作性和效率之间的平衡。单旋翼无人机虽然更复杂且成本更高，但为特定任务（如详细的地形测量）提供了卓越的精确度。最后，多旋翼无人机，尤其是四旋翼无人机，因其敏捷性、垂直起降能力和常用于监控和航空摄影应用而受到高度重视。多旋翼无人机可以是三旋翼、四旋翼、六旋翼或八旋翼。


得益于各种技术和方法的发展，人工智能（AI）近几十年来经历了显著的进步。机器学习（ML）技术因其能够通过与环境的互动提供决策自主性而脱颖而出。ML是不断发展的数据科学领域的重要组成部分。通过使用统计方法，模型被训练以在各种基于数据挖掘的项目中进行分类或预测，从而发现关键信息。在这一演变的背景下，深度学习（DL）作为ML中的先进技术而出现，它由具有多层的人工神经网络组成。这些深层结构使我们能够处理复杂任务，如识别图像中的模式或理解自然语言。存在多种ML算法，根据一些基本标准对它们进行预先分类是有益的。其中一个最重要和最显著的标准与算法的训练方式有关。在这种分类中，区分了四种主要方法：监督学习、无监督学习以及强化学习（RL）。

计算机硬件和软件的最新发展使得人工智能（AI）成为几乎所有与工程相关的研究领域的关键组成部分。人工智能是解决那些没有明确答案或传统方法需要大量人为干预的难题的强大工具。人工智能与传统认知算法之间的一个显著区别在于，人工智能可以自动提取特征。这取代了昂贵的手工特征工程。无人机的许多问题，如高功率/能源消耗和实时需求，也是边缘计算和边缘人工智能的优点，如低能耗和低延迟。深度学习和无人机（UAV）有潜力彻底改变交通部门的交通监控。
...
这些3D目标检测算法在思想上与2D目标检测算法最为接近，仅以单目/立体图像作为输入来预测3D目标实例。一般分为两种：基于模板匹配的方法和基于几何属性的方法。

==== 基于模板匹配的方法

这种方法的主要思想是通过穷尽采样执行2D/3D匹配并将3D候选区与作为代表模板进行评分。典型例子就是早期由 Chen提出的3DOP，其对输入的立体图像估计深度，并将图像平面上的像素级坐标投影到三维空间来估计点云。3DOP将候选区生成的问题定义为Markov随机场（MRF）的能量最小化问题，该问题需要对势函数进行精心设计。获取3D目标的候选框后，3DOP使用FastR-CNN方案来回归预测目标位置。
在可能的情况下，汽车仅配备一个摄像头，Chen随后提出了Mono3D来通过单目相机达到PAR性能。与3DOP不同的是，Mono3D不计算深度信息，而是滑动窗口从3D空间采样3D候选区。当对对象所在地平面与图像平面正交时，可以减少搜索的工作量。3DOP或Mono3D输出制定类别的候选区，这需要为每个类别单独设计方案。然而过度依赖工程和专业领域知识使得模型在复杂场景下的通用性比较差。

==== 基于几何特性的方法

这种方法并不需要大量的候选区来提高召回率，而是直接从精确的2D边界框开始，直接从经验观察中获得几何特性来粗略的估计3D姿态。

Mousavian等人提出的Deeo3DBox利用几何特性，让3D角的透视投影至少紧密贴在2D边界框的一侧。Li等人提出了GS3D，仅使用单目RGB图像来检测完整的3D实例，而并没有引入额外的数据。GS3D基于Faster RCNN的框架，额外添加了一个名为2D+O的子网络，用于预测2D边界框和观测的方向。接着GS3D获得粗略的3D框，称为引导。最后，在被送3D子网络进一步细化之前，从2D框和3D框上选择3个可见面进行融合来解决表征模糊的问题。尽管GS3D有了显著的性能提升， 超过了现有的基于单目图像的方法，但其依赖于经验知识，而并不能保证是准确的，且很容易受到对象范围和大小的影响。

==== 基于伪激光雷达的方法

这些方法首先进行深度估计，然后求助于现有的基于点云的方法。

Xu等人提出了MF3D,该算法对图像特征和伪激光雷达进行多层次的融合。具体来说，MFD首先通过独立的单目深度估计模块计算视差，以获得伪激光雷达。同时，采用标准的 2D 候选区域生成网络，将 RGB 图像与由视差图获得的转换前视图特征融合作为输入。随着2D候选区域的获得，RGB图像和伪激光雷达的特征通过串联融合，以进一步细化。最近，Weng提出了Mono3D-PLiDAR,通过单目深度估计将输入图像升级成3D相机坐标，即伪激光雷达点(例如,DORN)。然后是3D目标检测其，称为Frustum PointNets,应用于伪激光雷达。Weng等人揭示了伪激光雷达由于单目深度估计的误差，会产生大量的噪声，这体现在两个方面：激光雷达点的局部不对准和深度伪影的问题。为了克服前者，Mono3D-PLiDAR使用2D-3D边界框一致性损失（BBCL)来有监督的训练。为了缓解后者问题，Mono3D-PLiDAR采用Mask R-CNN预测的实例掩码代替2D边界框来减少截锥内不相关的点。
...
Wang等人提出了DETR3D。该算法是DETR在三维目标检测领域的延伸，它通过几何反投影和相机变换矩阵将二维特征提取与三维目标预测联系起来，实现了无需密集深度估计的三维目标检测。DETR3D将多视图检测问题转化为集合到集合的预测任务，通过预设的object queries和神经网络解码出三维空间中的参考点，再将这些点反投影到二维特征图上，通过双线性插值采样特征值，最终通过多头注意力机制和Transformer解码器来优化queries并预测边界框和类别。然而DETR3D存在预测参考点不准确、无法从全局角度进行表示学习等缺点，为此，Liu和Wang等人提出了PETR。PETR通过引入3D坐标生成器和3D位置编码器，将三维坐标的位置信息编码为图像特征，从而实现多视图三维目标检测。PETR首先在三维空间中初始化一组均匀分布的anchor points，然后通过MLP网络生成初始对象查询。与DETR3D不同，PETR先预设三维坐标再编码到query，这样做避免了在图像平面找不到对应点的问题，并实现了在三维特征空间中的训练。之后Liu等人提出了PETRv2，PETRv2在PETR的基础上增加了时间建模，通过将前一帧的特征与当前帧的特征融合来捕获时间线索，简化了速度预测，并实现了不同帧目标位置的时间对齐。PETRv2通过特征引导的位置编码器将图像特征和三维位置信息结合，隐式引入了视觉先验，提高了模型的性能。

Huang等人提出的BEVDet是另一种高性能的多相机三维目标检测方法，它在鸟瞰图（BEV）空间中进行目标检测。BEVDet面临过拟合问题，因为它在BEV空间下过度拟合。为了解决这个问题，BEVDet应用了定制的数据增强策略和尺度NMS（Scale-NMS），以提高模型的泛化能力和检测性能。
之后Huang等人提出了BEVDet4D，BEVDet4D通过将前一帧的特征与当前帧中的相应特征融合来捕获时间线索，简化了速度预测，并实现了不同帧目标位置的时间对齐。
Li等人提出的BEVFormer利用可变形注意力机制设计了空间交叉注意力和时间自注意力，分别从跨摄像机视图的感兴趣区域提取空间特征和循环融合历史BEV信息，从而实现对三维场景的理解和目标检测。
...
= 项目研究的基本方法、实验方案及可行性分析

== 项目研究的基本方法和实验方案

=== 研究方法：

==== 文献阅读：阅读可控文本和图像生成的相关文献，掌握可控文本和图像生成的基本原理和常用方法；阅读视觉文本渲染的相关文献，掌握基于视觉文本的可控图像生成的最新方法和最新进展。

==== 代码实现：实验复现现有的可控文本和图像生成模型。在此基础上，引入一个新模块做二者融合，确定并实现本项目要使用的模型与方法。
